...
//			}
		...
		{	
			if (Config::getInstance()->debug === true)
			{
				if (!((get_class($exception) == 'Ajde_Exception' || is_subclass_of($exception, 'Ajde_Exception')) && !$exception->traceOnOutput())) {
					AjdeExceptionLog::logException($exception);				
					echo self::trace($exception);
				} else {
					AjdeExceptionLog::logException($exception);
					Response::redirectServerError();
				}
			}
			else
			{
	...
	public static function trace(Exception $exception, $output = self::EXCEPTION_TRACE_HTML)
	{
		...
		switch ($output) {
			case self::EXCEPTION_TRACE_HTML:
				...
				if ($exception instanceof AjdeException && $exception->getCode()) {
					$exceptionDocumentation = sprintf("<div style='margin-top: 4px;'><img src='" . Config::get('site_root') . MEDIA_DIR . "_core/globe_16.png' style='vertical-align: bottom;' title='Primary key' width='16' height='16' /> <a href='%s'>Documentation on error %s</a>&nbsp;</div>",
						Documentation::getUrl($exception->getCode()),
						$exception->getCode()
					);
				}
    ...
    public static function getTypeDescription(Exception $exception)
    {
        ...
        } elseif ($exception instanceof AjdeException) {
    ...
    {
        if ($exception instanceof ErrorException) {
            return AjdeLog::CHANNEL_ERROR;
        } elseif ($exception instanceof Routing) {
            return AjdeLog::CHANNEL_ROUTING;
        } elseif ($exception instanceof Security) {
            return AjdeLog::CHANNEL_SECURITY;
        } elseif ($exception instanceof AjdeException) {
            return AjdeLog::CHANNEL_APPLICATION;
        } else {
            return AjdeLog::CHANNEL_EXCEPTION;
        }
    }
    ...
    {
        if ($exception instanceof ErrorException) {
            return AjdeLog::LEVEL_ERROR;
        } elseif ($exception instanceof Routing) {
            return AjdeLog::LEVEL_WARNING;
        } elseif ($exception instanceof Security) {
            return AjdeLog::LEVEL_WARNING;
        } elseif ($exception instanceof AjdeException) {
            return AjdeLog::LEVEL_ERROR;
