...
class Html extends Document
{
	...
	public function renderCompressedResources(array $types = array('*'))
	{
		...
		foreach ($this->getResources() as $resource)
		{
			...
			if (current($types) == '*' || in_array($resource->getType(), $types))
			{				
				if ($resource instanceof Local && !$resource->hasNotEmpty('arguments'))
				{
					if (!isset($this->_compressors[$resource->getType()]))
					{
						$this->_compressors[$resource->getType()] =
								Compressor::fromType($resource->getType());
					}
	...
	public function autoAddResources(Template $template)
	{
		$position = $template->getDefaultResourcePosition();
		foreach($this->getResourceTypes() as $resourceType) {
            // default resource
			if ($defaultResource = Local::lazyCreate($resourceType, $template->getBase(), 'default', $template->getFormat())) {
				$this->addResource($defaultResource, $position);
			}
            // default sub-action resource
			if (substr_count($template->getAction(), '/') > 0 &&
    				$actionDefaultResource = Local::lazyCreate($resourceType, $template->getBase(), $this->_getTemplateActionDefault($template), $template->getFormat())) {
				$this->addResource($actionDefaultResource, $position);
			}
            // non-default resource
            if ($template->getAction() != 'default' &&
			    	$actionResource = Local::lazyCreate($resourceType, $template->getBase(), $template->getAction(), $template->getFormat())) {
