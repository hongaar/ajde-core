...
class Curl {
	...
	static public function get($url, $toFile = false, $header = false)
    {
        ...
        if ($debug) Log::_('cURL URL', Log::CHANNEL_INFO, Log::LEVEL_INFORMATIONAL, $url);
		...
		try {
			...
			if ($toFile !== false) {
                ...
                if ($debug) Log::_('cURL result', Log::CHANNEL_INFO, Log::LEVEL_INFORMATIONAL, var_export($verbose, true));
