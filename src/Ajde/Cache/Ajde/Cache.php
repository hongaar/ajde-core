...
class Cache extends Singleton
{
	...
	public function saveResponse()
	{
		...
		if ($document->getCacheControl() == Document::CACHE_CONTROL_NOCACHE || $this->isEnabled() == false) {
			$response->addHeader('Expires', gmdate('D, d M Y H:i:s \G\M\T', time()));
			$response->addHeader('Cache-Control', Document::CACHE_CONTROL_NOCACHE . ', max-age=0');
		} else {
		...
		if (($this->ETagMatch() && !$this->modifiedSince()) && $this->isEnabled()) {
			$response->setResponseType(Response::RESPONSE_TYPE_NOT_MODIFIED);			
